...
int RecordManager::Record_DeleteAllValue(const string table_name, vector<Attribute>& Attribute_vec)
{
    ...
    int ret = get_Record_Size(table_name, Attribute_vec)+1;
    chunk->pin();
    chunk->tail = 0;
    memcpy(chunk->data, &(chunk->tail), 4);
    chunk->unPin();
  		...
  		if(flag)
  		{
            ...
            int tail_addr = blocktmp->tail+4;
  			...
  			if(tail_addr == 4)
  			{
                ...
                memcpy(&(blocktmp->tail), blocktmp->data, 4);
  				tail_addr = -blocktmp->tail+4;
                blocktmp->tail = -blocktmp->tail;
...
{
    string path = CL.used_Database + "+" + table_name + "+" + attribute_name + "+" + table_name + "-" + attribute_name + "-index";
    int bid = 0;
    Block* chunk = bm.getBlock(table_name, bid);
    //    if(chunk->tail==0)
    //        return;
...
//    cout<<query_addr<<endl;
    return query_addr==(abs(chunk->tail+4))?true:false;
}
//--------------for class condition-------------// 
